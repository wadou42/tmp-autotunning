...
class RedisManager:
    def __init__(
        ...
        # os.sched_setaffinity(0, {int(self.redis_start_core), int(self.redis_bench_core)})z
    ...
    def build(self, opt_config: str = "-g -O3"):
        ...
        make_env["OPTIMIZATION"] = f"-g -O3 -flto=12 {opt_config}"
...
if __name__ == "__main__":
    ...
    #     "-O3",
    #     "-O3 -flto=12",
    #     "-O3 -flto=12 -freschedule-modulo-scheduled-loops -flive-range-shrinkage --param max-inline-insns-auto=138 --param inline-unit-growth=397 --param max-inline-recursive-depth-auto=20 --param large-function-insns=6955 --param large-function-growth=861 --param large-unit-insns=38034 --param ipa-cp-eval-threshold=1184 --param ipa-cp-loop-hint-bonus=67 --param ipa-cp-recursion-penalty=40 --param ipa-cp-single-call-penalty=62 --param ipa-cp-value-list-size=4 --param max-unroll-times=224 --param max-unrolled-insns=3262 --param max-unswitch-insns=1584 --param max-average-unrolled-insns=501 --param max-completely-peel-loop-nest-depth=52 --param max-completely-peel-times=118 --param max-completely-peeled-insns=1426 --param unroll-jam-max-unroll=4",
    #     # "-O3 -flto=12 -freschedule-modulo-scheduled-loops -flive-range-shrinkage",
    #     # "-O3 -flto=12 --param max-inline-insns-auto=138 --param inline-unit-growth=397 --param max-inline-recursive-depth-auto=20 --param large-function-insns=6955 --param large-function-growth=861 --param large-unit-insns=38034 --param ipa-cp-eval-threshold=1184 --param ipa-cp-loop-hint-bonus=67 --param ipa-cp-recursion-penalty=40 --param ipa-cp-single-call-penalty=62 --param ipa-cp-value-list-size=4 --param max-unroll-times=224 --param max-unrolled-insns=3262 --param max-unswitch-insns=1584 --param max-average-unrolled-insns=501 --param max-completely-peel-loop-nest-depth=52 --param max-completely-peel-times=118 --param max-completely-peeled-insns=1426 --param unroll-jam-max-unroll=4",
    # ]
    # # numactl -N 3 -m 3 taskset -c 311  ./redis-server /home/whq/dataset/redis/redis/redis/redis.sock;
    # # numactl -N 3 -m 3 taskset -c 319  ./redis-benchmark -s /home/whq/dataset/redis/redis/redis/redis.sock -d 3 -n 100000000 -P 100 -c 80 -q get,set
    # for flag in flags:
    #     print(f"Building Redis with flags: {flag}", flush=True)
    #     current_time = time.time()
    #     redis_manager.build(opt_config=flag)
        
    #     result: float = 0
    #     result = redis_manager.test()
    #     print(f"Test result: {result:.4f} us", flush=True)
    #     test_time = time.time() - current_time
    #     print(f"Total time for build and test with flags [{flag}]: {test_time/60:.2f} minutes", flush=True)
