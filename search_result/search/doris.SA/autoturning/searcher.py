...
class Searcher(ABC):
    ...
    def __init__(self,
        ...
        self.ai4c_writer = AI4CConfigWriter(ai4c_metadata, search_config.ai4c_config_dir)
        ...
        if ai4c_metadata is not None:
            for func_hash in ai4c_metadata.func_metadata.keys:
                for param_name in ai4c_metadata.param_metadata.keys:
                    for _ in range(ai4c_metadata.param_metadata.elems[param_name].width):
                        self.param_metadatas.append(ai4c_metadata.param_metadata.elems[param_name])
    ...
    def init_evaluate(self) -> list[float]:
        ...
        for opt_setting in self.opt_settings:
            ...
            if len(opt_performance) == self.population:
                break
        assert len(opt_performance) == self.population, \
    ...
    def evaluate(self, opt: list) -> float:
        ...
        if self.constrains_solver is not None:
            self.constrains_solver.solve(opt_config=option_config, ai4c_config=ai4c_config)
            new_c: list[int] = encode(ai4c_config=ai4c_config,
        ...
        option_config_filted: dict[str, Union[str, bool, int, list]] = {
            k: v for k, v in option_config.items() if not (isinstance(v, str) and v == 'OFF')
        }
        ...
        ai4c_part = ''
        if yaml_file != '':
            ai4c_part = f'-fplugin={self.search_config.ai4c_plagin_lib} ' \
        ...
        # record the evaluation.
        self.perf_recorder.record(f'{self.perf_recorder.iter}: O3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
...
class MutateBasedSearcher(Searcher, ABC):
    def __init__(self,
                 ...
                 search_config: SearchConfig,
                 constrains_file: str):
        super().__init__(ai4c_metadata=ai4c_metadata,
    ...
    def mutation(self, opt: list[int]) -> list[int]:
        mutate_idx: list[int]
        new_values: list[int]
        mutate_idx = random.sample(range(len(opt)), self.mutation_bits)
        new_values = [mutate_to_new_code(opt[_], self.param_metadatas[_]) if _ in mutate_idx else opt[_]
...
class RandomSearcher(MutateBasedSearcher):
    ...
    def search(self):
        ...
        while self.evaluation_cnt_limit != 0 and self.time_limit > 0:
            ...
            print(f'[RandomSearcher.search] remain evaluation count: {self.evaluation_cnt_limit}')
...
class GASearcher(MutateBasedSearcher):
    ...
    def selection(self, opt_settings, performances) -> tuple[list[list[int]], list[float]]:
        """
        ...
        opt_performance = sorted([[performances[_], opt_settings[_]] for _ in range(len(performances))],
    ...
    def search(self):
        ...
        while True:
            print(f'[GASearcher.search] remain evaluation count: {self.evaluation_cnt_limit}')
            start_time = time.time()
            ...
            for idx in range(len(population) - 1):
                new_population.extend(self.crossover(population[idx], population[idx + 1]))
...
class PSOSearcher(Searcher):
    ...
    def search(self):
        ...
        while True:
            ...
            print(f'[PSOSearcher.search] remain evaluation count: {self.evaluation_cnt_limit}')
            if self.evaluation_cnt_limit <= 0 or self.time_limit <= 0:
                break
            # update velocity
            r1 = float(random.randint(0, 100)) / 100.0
            r2 = float(random.randint(0, 100)) / 100.0
            velocities = np.array([self.w * velocities[_] +
            ...
            # update position
            new_particles = [particles[_] + velocities[_] for _ in range(len(particles))]
            # new_particles = [[__ if 0.0 <= __ <= 1.0 else (0.0 if __ < 0.0 else 1.0) for __ in _] for _ in new_particles]

            # formalize
            velocities = np.array([[float(formalize_code_bit(_[idx], self.param_metadatas[idx])) * self.v_rate
                                    for idx in range(len(_))]
                                   for _ in velocities])
            new_particles = [[float(formalize_code_bit(_[idx], self.param_metadatas[idx]))
                              for idx in range(len(_))]
    ...
    def search(self):
        ...
        while True:
            ...
            for idx in range(len(current_state)):
                print(f'[SASearcher.search] remain evaluation count: {self.evaluation_cnt_limit}')
                if self.evaluation_cnt_limit <= 0 or self.time_limit <= 0:
                    break
                ...
                if (delta_energy > 0 or random.random() < math.exp(delta_energy / self.temperature)) and new_energy > 0:
...
class DESearcher(Searcher):
    ...
    def selection(self, opt_settings, performances) -> tuple[list[list[int]], list[float]]:
        ...
        if len(opt_settings) == self.population:
            return opt_settings, performances
        opt_performance = sorted([[performances[_], opt_settings[_]]
                                  for _ in range(len(performances))],
                                 reverse=True)[:self.population]
        return [_[1] for _ in opt_performance], [_[0] for _ in opt_performance]
    ...
    def search(self):
        ...
        while True:
            ...
            print(f'[DESearcher.search] remain evaluation count: {self.evaluation_cnt_limit}')
            if self.evaluation_cnt_limit <= 0 or self.time_limit <= 0:
                break
            ...
            for idx in range(len(population)):
                ...
                # mutation
                i1 = idx
                i2, i3 = random.sample([_ for _ in range(len(population)) if _ != i1], 2)
                opt1 = population[i1]
                opt2 = population[i2]
                opt3 = population[i3]
                ...
                new_pop = [mutated[_] if _ in crossover_idx else opt1[_] for _ in range(len(opt1))]
...
if __name__ == '__main__':
    ...
    ai4c_option_file = '/usr/local/lib/python3.9/site-packages/ai4c/autotuner/yaml/coarse_options.yaml'
    ...
    search_config: SearchConfig = SearchConfig(ai4c_config_dir='../yamls',
    ...
    doris_evaluator: Evaluator = DorisEvaluator(o3_project_path=o3_project_path,
    ...
    random_searcher = RandomSearcher(opt_settings=opt_settings,
                                     population=5,
