...
from autoturning.manager.rocksdb_manager import RocksDBManager
from autoturning.manager.zstd_manager import ZstdManager
from autoturning.manager.mysql_manager import MySQLManager
...
class RedisEvaluator(Evaluator):
    ...
    def evaluate_old(self, opt: str) -> tuple[float, float, float]:
        ...
        if mode == 'O3':
            o3_manager = RedisManager(redis_home=self.o3_project_path, enable_gperftools=False)
            opt_manager = RedisManager(redis_home=self.opt_project_path, enable_gperftools=False)
...
class ScannEvaluator(Evaluator):
    ...
    def evaluate(self, opt: str) -> tuple[float, float, float]:
        ...
        return o3_perf, opt_perf, calculate_acc_rate(o3_perf=o3_perf, opt_perf=opt_perf, project='scann')
    ...
    def evaluate_build_once(self, opt: str, recorder: Recorder, repeat_time: int):
        ...
        for idx in range(repeat_time):
            opt_perf = manager.test()
            acc_rate = calculate_acc_rate(o3_perf, opt_perf, project="scann")
            recorder.record(f'iter={idx}, o3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
            if acc_rate != float('inf') and acc_rate > -1:
                all_acc_rate.append(acc_rate)
                all_opt_perf.append(opt_perf)
...
class RocksDBEvaluator(Evaluator):
    ...
    def evaluate(self,  opt: str) -> tuple[float, float, float]:
        rocksdb_manager = RocksDBManager(
            test_home=self.opt_project_path,
        )

        o3_time: Optional[float] = None
        opt_time: Optional[float] = None
        if autoturning.script_args.evaluate_search:
            rocksdb_manager.build(opt_config=opt)
            opt_time = rocksdb_manager.test()
            o3_time = autoturning.script_args.default_o3_perf
        if autoturning.script_args.evaluate_o3:
            rocksdb_manager.build(opt_config='-O3')
            o3_time = rocksdb_manager.test()
        assert o3_time is not None
        assert opt_time is not None
        return o3_time, opt_time, calculate_acc_rate(o3_perf=o3_time, opt_perf=opt_time, project='rocksdb')

    def evaluate_build_once(self, opt: str, recorder: Recorder, repeat_time: int):
        manager = RocksDBManager(
            build_home=self.opt_project_path,
            install_home=self.install_home,
            db_home=self.db_home,
        )
        manager.build(opt_config=opt)
        ...
        for idx in range(repeat_time):
            opt_perf = manager.test()
            acc_rate = calculate_acc_rate(o3_perf, opt_perf, project="rocksdb")
            recorder.record(f'iter={idx}, o3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
            if acc_rate != float('inf') and acc_rate > -1:
                all_acc_rate.append(acc_rate)
                all_opt_perf.append(opt_perf)
...
class ZstdEvaluator(Evaluator):
    ...
    def evaluate(self,  opt: str) -> tuple[float, float, float]:
        zstd_manager = ZstdManager(
            build_dir=self.opt_project_path,
        ...
        opt_time: Optional[float] = None
        if autoturning.script_args.evaluate_search:
            zstd_manager.build(opt_config=opt)
            opt_time = zstd_manager.test()
            o3_time = autoturning.script_args.default_o3_perf
        if autoturning.script_args.evaluate_o3:
            zstd_manager.build(opt_config='-O3')
            o3_time = zstd_manager.test()
        assert o3_time is not None
        assert opt_time is not None
        return o3_time, opt_time, calculate_acc_rate(o3_perf=o3_time, opt_perf=opt_time, project='zstd')
    ...
    def evaluate_build_once(self, opt: str, recorder: Recorder, repeat_time: int):
        ...
        for idx in range(repeat_time):
            opt_perf = manager.test()
            acc_rate = calculate_acc_rate(o3_perf, opt_perf, project="zstd")
            recorder.record(f'iter={idx}, o3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
            if acc_rate != float('inf') and acc_rate > -1:
                all_acc_rate.append(acc_rate)
                all_opt_perf.append(opt_perf)
...
class MysqlEvaluator(Evaluator):
    def __init__(self,
                 o3_project_path: str,
                 opt_project_path: str,
                 repeat_time: int,
    ...
    def evaluate(self,  opt: str) -> tuple[float, float, float]:
        ...
        opt_time: Optional[float] = None
        if autoturning.script_args.evaluate_search:
            mysql_manager.build(opt_config=opt)
            opt_time = mysql_manager.test()
            o3_time = autoturning.script_args.default_o3_perf
        if autoturning.script_args.evaluate_o3:
            mysql_manager.build(opt_config='-O3')
            o3_time = mysql_manager.test()
        assert o3_time is not None
        assert opt_time is not None
        return o3_time, opt_time, calculate_acc_rate(o3_perf=o3_time, opt_perf=opt_time, project='mysql')
    ...
    def evaluate_build_once(self, opt: str, recorder: Recorder, repeat_time: int):
        ...
        for idx in range(repeat_time):
            opt_perf = manager.test()
            acc_rate = calculate_acc_rate(o3_perf, opt_perf, project="mysql")
            recorder.record(f'iter={idx}, o3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
            if acc_rate != float('inf') and acc_rate > -1:
                all_acc_rate.append(acc_rate)
                all_opt_perf.append(opt_perf)
...
class RandomEvaluator(Evaluator):
    ...
    def evaluate_build_once(self, opt: str, recorder: Recorder, repeat_time: int):
        ...
        for idx in range(repeat_time):
            o3_perf, opt_perf, acc_rate = self.evaluate(opt)
            recorder.record(f'iter={idx}, o3_perf={o3_perf}, opt_perf={opt_perf}, acc_rate={acc_rate}\n')
            if acc_rate != float('inf') and acc_rate > -1:
                all_acc_rate.append(acc_rate)
                all_opt_perf.append(opt_perf)
